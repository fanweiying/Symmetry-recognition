...
#model=tf.keras.models.load_model(r'./2D_symmetry_calssfication/Results of CNN-II/Adam/model/CNN-II-Adam-001.h5')
test_dir = pathlib.Path(r'./2D_symmetry_calssfication/Dataset 2./test')
...
#The parameter y_true is the true label of the test data set, and y_pred is the prediction result of the network on the test data set 
...
#    plt.rc('font', **font)
    ...
    for i, j in itertools.product(range(cm.shape[0]), range(cm.shape[1])):
        if cm[i, j]>=0.02:
            plt.text(j, i, format(cm[i, j], fmt),
                     ...
                     verticalalignment="center",      ##需要减小一下字宽
    ...
    plt.tight_layout()
    if save_flg:
        plt.savefig("./2D_symmetry_calssfication/Results of CNN-II/confusion_matrix_CNN-II.pdf") 
    plt.show()
...
plot_confusion_matrix(cm,range(len(test_label_names)),normalize=True, title='confusion matrix')
