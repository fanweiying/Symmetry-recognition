# -*- coding: utf-8 -*-
"""
Created on Thu Mar  3 14:37:26 2022
...
Optimizer=optimizers.RMSprop(lr=0.001)  ##### or 'Optimizer=optimizers.Adam(lr=0.001)' or ' Optimizer=optimizers.RMSprop(lr=0.001)'
Optimizer_name='RMSprop'     ##### or 'Adam' or 'RMSprop'
epochs=50 


#%%
train_dir=os.path.abspath('./2D_symmetry_calssfication/Dataset 1./train')
...
#%%
validate_dir=os.path.abspath('./2D_symmetry_calssfication/Dataset 1./validate')
...
def create_model():
    ...
    model.compile(optimizer=optimizers.Adam(lr=0.001),loss='sparse_categorical_crossentropy', metrics=['acc'])
    return model
...
#%%
#model.compile(metrics=['acc','val_acc'])
print("optimizers：",Optimizer_name)
print('epochs=',epochs)
model_name='CNN-I-'+Optimizer_name+'-{epoch:03d}.h5'
train_model_dir='./2D_symmetry_calssfication./Results of CNN-I/'+Optimizer_name+'/model'
model_path=os.path.join(train_model_dir,model_name)
checkpoint=tf.keras.callbacks.ModelCheckpoint(filepath=model_path,monitor='val_acc',verbose=2,save_best_only=True,model='max',period=1)
...
#%%
f = open(r"./2D_symmetry_calssfication/Results of CNN-I/"+Optimizer_name+'_cost','a', encoding='utf-8')
...
plt.savefig('./2D_symmetry_calssfication./Results of CNN-I/'+Optimizer_name+'-acc.pdf')
plt.figure() 
plt.show()
...
plt.savefig('./2D_symmetry_calssfication./Results of CNN-I/'+Optimizer_name+'-loss.pdf')
plt.show()
#%%
